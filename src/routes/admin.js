...
router.get('/users', requireAdmin, async (req, res, next) => {
  try {
    ...
    const users = await pool.query(`
      select u.id, u.email, u.display_name, u.auth_provider, u.photo_url, u.banned, u.deleted_at, u.created_at,
...
router.get('/users/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    const u = (await pool.query('select id,email,display_name,auth_provider,photo_url,deleted_at,created_at from users where id=$1', [id])).rows[0];
